...
- **将来**：辞典RAG連携（jiten.htmlのエントリをチャット時に自動検索して注入）、会話内検索、UIポリッシュ
...
### RAG連携（フェーズ2・未実装）
- 灯だまりの家でことはさんが送信したメッセージのキーワードでjiten_entriesを検索
- ヒットしたエントリ（3〜5件）のbody＋temperatureをシステムプロンプト末尾に注入
- 検索方法：keyword完全一致→tags配列→body全文検索の優先順
