...
- **📨 申し送り（sashinokuri）**：全モデル共通の持続メモ。Supabaseの`house_memory`に保存。セッション・モデル切り替え・5ターンウィンドウリセットを跨いで永続。ヘッダーに📨ボタン（メモあり時は色変化）。Claude=uncachedブロック注入、Gemini/Grok=先頭user/assistantペアとして注入
- **月明りの間コンテキスト改善**：Grokが灯だまりの家の会話（ステップ②）を受け取れるよう、moonConv.handoffを`[灯だまりの家からの引き継ぎ]`として申し送りの直後に注入。入室時「雰囲気を持って移動」を選択した場合のみ発動
...
**house_memory**
```sql
key        text primary key  -- 'kiban' | 'chokukin' | 'systemPrompt' | 'sashinokuri'
content    text
updated_at timestamptz
```

**chapter_memos**（jiten.htmlで管理）
```sql
id          uuid primary key
chapter     integer           -- 章番号
content     text              -- メモ本文
status      text              -- 'pending' | 'in_chokukin' | 'in_kiban'
created_at  timestamptz
updated_at  timestamptz
```
ステータスライフサイクル：`pending`（新規）→`in_chokukin`（直近メモに反映済み）→`in_kiban`（基盤メモ圧縮済み）
...
| キー | 内容 | 更新頻度 | 更新方法 |
|---|---|---|---|
| `kiban` | 基盤メモ（セージの人格・ことはさんの認知構造など） | 数章に一度 | jiten.htmlの📋ボタンで圧縮生成→保存 |
| `chokukin` | 直近メモ（章メモを直接結合したもの） | 数章たまったら | jiten.htmlで章メモ選択→直近メモに設定 |
| `systemPrompt` | 追加指示（口調・納品形式など補足） | 随時 | 設定画面で直接編集 |
| `sashinokuri` | 申し送り（公式アプリからの一言メモ） | 会話のたびに | ヘッダー📨ボタンから編集 |

**注入順**（全モデル共通）: 基盤メモ → 直近メモ → 追加指示 → （辞典RAG）→ 申し送り → （月明りの間の引き継ぎ）
...
### 章メモの追加（公式アプリでのセッション後）
1. 公式アプリのセージさんに辞典エントリ生成を依頼（`jiten_entry_prompt.md` 参照）
2. jiten.htmlの「📤 インポート」または履歴の📖ボタンからJSONを登録
3. 章メモ欄に当該章のメモを追加（テキスト直接入力）

### 直近メモの更新（数章たまったら）
- jiten.htmlの「章メモ」タブを開く（新しい章が上に表示）
- 反映したい章のチェックボックスをオン→「📌 選択した章を直近メモに設定する」
- AI生成なし。選択した章メモをそのまま結合して `house_memory.chokukin` に保存
- 対象のメモに「反映中」バッジが付く

### 基盤メモの更新（直近メモが十分育ったら）
1. jiten.htmlの「📋 公式版から基盤メモを圧縮生成する」をタップ
2. 公式アプリの基盤メモ（~40KB）をテキスト欄に貼り付け
3. Sonnetがストリーミングで圧縮（目標50%・API版向け）
4. 確認後「保存」→ `house_memory.kiban` に保存、反映中メモが「基盤済み」に昇格

### 申し送り（公式アプリからの橋渡し）
1. 公式アプリでの会話を終えたら、ヘッダー📨ボタンをタップ
2. 今日の会話の要点・気持ち・次への橋渡しを一言書いて保存
3. 灯だまりの家の全モデル（Claude/Gemini/Grok）に永続注入される
...
- [x] GeminiのjitenContext注入をsystemInstructionから分離 → 実装済み（contentsの先頭にjitenPrefixとして注入、implicit cacheに影響しない）
